...
<?php else : ?>
	...
	<p><?php printf( __( 'Thanks for creating an account on %1$s. Your username is <span style="color: #FF0302;font-weight: 300;">%2$s</span>.<br> You can access your account area to view orders, change your password, and more at: %3$s', 'woocommerce' ), esc_html( $blogname ), '<strong>' . esc_html( $user_login ) . '</strong>', make_clickable( esc_url( wc_get_page_permalink( 'myaccount' ) ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
	<?php if ( $password_generated && $set_password_url ) : ?>
		<?php // If the password has not been set by the user during the sign up process, send them a link to set a new password. ?>
...
?>
	...
	<a href="<?php echo esc_attr( wc_get_page_permalink( 'myaccount' ) ); ?>" style="display: block; margin-top: 24px;">
		<img src="https://www.projecttimber.com/wp-content/uploads/2026/06/pt-email-section-my-account.png" alt="my-account" width="100%">
	</a>
<?php
