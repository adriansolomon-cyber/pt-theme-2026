...
foreach ( $items as $item_id => $item ) :
	if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
		...
		$line_subtotal = wc_price( ( wc_get_order_item_meta( $item_id, '_line_total' ) ) * 1.2 );		
		$component_title = trim( get_component_title_by_order_item( $item->get_id(), $item ) );
		?>
		<tr class="order_item-details <?php echo ( "" === $component_title ? 'parent' : 'child' );?>">
						...
						<td class="order_items_table_product_details_inner_td order_items_table_product_details_inner_td_text" width="100%">
							...
							<div class="order_items_table_product_details_inner_title">
								...
								if( $component_title <> "" ) {
									?>
									<dl class="component">
										<dt class="component-title"><?php echo ucwords( $component_title ); ?>:</dt>
										<dd class="component-value"><?php echo $product->get_title(); ?></dd>
									</dl>
									<?php
								} else {
