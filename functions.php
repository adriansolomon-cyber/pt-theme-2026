...
require_once get_stylesheet_directory() . '/includes/contact-form.php';        // Contact page enquiry form handler
...
/**
 * Klaviyo renders its SMS/mobile consent disclosure at the bottom of the billing
 * form (woocommerce_after_checkout_billing_form). Our form-billing.php moves the
 * consent up into the Contact block (styled with the "Read more" toggle) instead,
 * so drop the default bottom placement to avoid it showing twice / unstyled.
 * The plugin's mobile-channels update renamed the callback kl_sms_compliance_text
 * → kl_mobile_compliance_text; remove both names so it works across versions.
 ...
 * Resources, Building a Base). Each uses a NAMED page template in templates/
 * (Template Name: … assigned via Page Attributes → Template), so it doesn't
 * depend on the page slug. They share the hero/breadcrumb + help-CTA components
 * styled by secondary.css.
 *
 * This detects our secondary templates by their path (templates/page-*.php) and
 * auto-loads a per-template stylesheet/script derived from the filename —
 * templates/page-faq.php → assets/css/faq.css + assets/js/faq.js — so adding
 * each page later needs no functions.php change.
 *
...
add_action(
	...
	function () {
		...
		if ( 0 !== strpos( $tpl, 'templates/page-' ) ) {
			return; // Not one of our secondary templates.
		}
		...
		// Per-template assets: templates/page-faq.php → "faq".
		$name = preg_replace( '/^page-/', '', basename( $tpl, '.php' ) );
		if ( $name ) {
			$css_rel = 'assets/css/' . $name . '.css';
			if ( file_exists( $dir . '/' . $css_rel ) ) {
				wp_enqueue_style( 'pt-page-' . $name, $uri . '/' . $css_rel, array( 'pt-secondary' ), $ver( $css_rel ) );
			}
			$js_rel = 'assets/js/' . $name . '.js';
			if ( file_exists( $dir . '/' . $js_rel ) ) {
				wp_enqueue_script( 'pt-page-' . $name, $uri . '/' . $js_rel, array(), $ver( $js_rel ), true );
			}
		}
	},
	15
